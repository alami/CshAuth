    ...
    {     
        ...
        public async Task<IActionResult> OnPostAsync(string? returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded )
                {
                    _logger.LogInformation("News user created.");
                    
                    var code = await _userManager.GenerateEmailConfirmationTokenAsync(user); //1-31-14
                    //code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
                    var callbackUrl = Url.Page(
                        "/Account/ConfirmEmail",
                        pageHandler: null,
                        values: new { userId = user.Id, code = code }, //, area = "Identity", returnUrl = returnUrl },
                    ...
                    {
                        return LocalRedirect("~/");  //to the root
                    }
                    else
                    {
